...
- Git credentials are entered locally and the PAT is stored through `keyring`'s native store. On Windows this selects the Windows credential store; on Android the keyring crate's native Android store is used. The native store is accessed only when credentials are saved or retrieved.
